 ...
 * 
 * 使用方法：
 * node auto_scan_and_add_to_bookshelf.js
 */
...
const LOCAL_COMIC_DIRS = [
  path.join(process.env.HOME, 'Documents', 'ComicDownloads'),
  path.join(process.env.HOME, 'Downloads', 'comic-app', 'downloads'),
  // 添加更多目录...
]
...
async function main() {
  try {
    ...
    // 导入到书架
    await importComicsToBookshelf(allComics, targetDir)
