...
let cleanupUpdateDone = null
let cleanupJobDoneNotif = null
let cleanupJobFailedNotif = null
...
onMounted(() => {
  ...
  // crawler update-done
  if (window.crawlerApi?.onUpdateDone) {
    cleanupUpdateDone = window.crawlerApi.onUpdateDone((data) => {
      addNotification('info', '追更检查完成', `更新 ${data?.updated || 0} 部 · 新章入队 ${data?.queuedChapters || 0} 章`)
    })
  }

  // job://done / failed 事件 → toast（来自 Rust jobqueue）
  if (window.downloadApi?.onJobDone) {
    cleanupJobDoneNotif = window.downloadApi.onJobDone((data) => {
      const label = taskTypeLabels[data?.type] || '下载任务'
      const title = `${label} 完成`
      const body = data?.comicTitle || data?.title || ''
      addNotification('success', title, body)
    })
  }
  if (window.downloadApi?.onJobFailed) {
    cleanupJobFailedNotif = window.downloadApi.onJobFailed((data) => {
      const label = taskTypeLabels[data?.type] || '下载任务'
      addNotification('error', `${label} 失败`, data?.error || data?.msg || '未知错误')
    })
  }

  // 全局 toast（简单版：window.dispatchEvent('toast'）
  ...
  autoEnrichTimer = setTimeout(async () => {
    try {
      if (window.detailApi?.autoEnrichAll) {
        ...
        if (result.enriched > 0 || result.total === 0) {
          addNotification('success', '字段补全完成', `成功补全 ${result.enriched || 0} 部漫画的所有字段`)
        } else if (result.failed > 0) {
          addNotification('warning', '字段补全部分失败', `成功 ${result.enriched} / 失败 ${result.failed} / 共 ${result.total}`)
        }
      }
    } catch (e) {
  ...
  // 延迟30秒后自动追更同步（等 enrich 先跑完，避免同时大量网络请求）
  autoSyncTimer = setTimeout(async () => {
    try {
      if (window.detailApi?.syncNow) {
        console.log('[Auto Sync] 开始自动追更同步...')
        const result = await window.detailApi.syncNow()
        if (result.queuedChapters > 0) {
          addNotification('success', '追更同步完成', `发现 ${result.updated || 0} 部漫画有新章节，入队 ${result.queuedChapters} 章`)
        } else if (result.checked > 0) {
          console.log(`[Auto Sync] 同步完成: 检查 ${result.checked} 本，无新章`)
        }
      }
    } catch (e) {
      console.warn('[Auto Sync] 同步失败:', e)
    }
  }, 30000)

  // 延迟5秒后自动扫描本地目录（如果设置里开启了 autoScanOnStartup）
  autoScanTimer = setTimeout(async () => {
    try {
      const settings = await window.settingsApi?.get?.()
      if (settings?.autoScanOnStartup && settings?.autoScanPaths?.length > 0) {
        console.log('[Auto Scan] 开始自动扫描本地目录...')
        if (window.dbApi?.autoScanLocalComics) {
          const result = await window.dbApi.autoScanLocalComics(settings.autoScanPaths)
          if (result.total > 0) {
            addNotification('success', '本地扫描完成', `发现 ${result.total} 部漫画，注册 ${result.chaptersRegistered || 0} 章`)
          }
        }
      }
    } catch (e) {
...
onBeforeUnmount(() => {
  ...
  if (cleanupUpdateDone) cleanupUpdateDone()
  if (cleanupJobDoneNotif) cleanupJobDoneNotif()
  if (cleanupJobFailedNotif) cleanupJobFailedNotif()
  if (downloadSpeedTimer) clearTimeout(downloadSpeedTimer)
...
function handleToast(e) {
  const m = (e && e.detail) ? String(e.detail) : ''
  if (m) addNotification('info', '', m)
}
