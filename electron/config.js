...
module.exports = {
  JOB_QUEUE: {
    ...
    AGING_ENABLED: env('JOB_AGING', true),
    AGING_INTERVAL: env('JOB_AGING_INTERVAL', 5 * 60 * 1000), // 每 5 分钟老化一级
    AGING_MIN_PRIORITY: env('JOB_AGING_MIN', 3), // 老化最高升到 priority=3 (不超手动任务)
    DYNAMIC_CONCURRENCY: env('JOB_DYNAMIC_CONCURRENCY', true),
    DYNAMIC_CONCURRENCY_INTERVAL: env('JOB_DYNAMIC_INTERVAL', 30 * 1000), // 30s 采样
    DYNAMIC_CONCURRENCY_MIN: env('JOB_DYNAMIC_MIN', 3),
    DYNAMIC_CONCURRENCY_MAX: env('JOB_DYNAMIC_MAX', 7),
    DYNAMIC_CPU_HIGH: env('JOB_DYNAMIC_CPU_HIGH', 0.8),
    DYNAMIC_CPU_LOW: env('JOB_DYNAMIC_CPU_LOW', 0.5),
    DYNAMIC_MEM_LOW_MB: env('JOB_DYNAMIC_MEM_LOW', 500),
    DYNAMIC_MEM_HIGH_MB: env('JOB_DYNAMIC_MEM_HIGH', 1024)
  },
